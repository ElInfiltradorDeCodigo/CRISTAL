...
export class AgregarVentaPage implements OnInit {
  ...
  generarPDF(ventaData: any) {
    ...
    const creditString = ventaData.mostrarCampos 
        ? [
            { text: `Total A Diferir: ${ventaData.totalDiferir}` },
            { text: `Pago Por Mes: ${ventaData.meses}` },
            { text: `Observaciones: ${ventaData.observaciones}` }
          ]
        : [{ text: `Venta a Crédito: NO` }];
    ...
    const documentDefinition = {
      content: [
            ...
            productosString,
            { text: `Total: ${ventaData.total}`, margin: [0, 10, 0, 0] },
            ...creditString
          ]
